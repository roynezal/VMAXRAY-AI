"""
VMAXRAY TUNNEL CLIENT for Win7 (GAS v2.0)
Run on Win7 alongside vmaxray_chat_server.py (port 8080)

Routes browser requests from GAS → Win7 chat server (port 8080)
"""
...
def run_diagnostics():
    log("=" * 50)
    log(" VMAXRAY TUNNEL WIN7 (GAS v2.0)")
    log("=" * 50)
    all_ok = True

    log("[1/2] Checking Win7 chat server...")
    try:
    ...
    r = post_json(relay_url('register'), {}, timeout=10)
    if r.get('ok'):
        log("      [OK] Register OK")
    else:
    ...
    while True:
        try:
            ...
            if now - last_keepalive >= KEEPALIVE_S:
                r = post_json(relay_url('register'), {}, timeout=8)
                if r.get('ok'):
                    log("Keepalive [OK]")
                else:
            ...
            result = post_json(relay_url('poll'), {}, timeout=POLL_INTERVAL + 5)
            ...
            resp['id'] = req_id
            send_r = post_json(relay_url('respond'), resp, timeout=15)
            if 'error' in send_r:
